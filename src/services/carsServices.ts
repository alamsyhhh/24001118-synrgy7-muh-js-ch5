...
class CarService implements ICarService {
  ...
  async createCar(req: Request): Promise<Cars | null> {
    ...
    if (!req.file.mimetype.startsWith('image/')) {
      throw new Error(
        'Hanya diperbolehkan mengunggah file gambar (JPG, PNG, GIF)'
      );
    }
  ...
  async updateCar(req: Request): Promise<Cars | null> {
  ...
  async deleteCarById(res: Response, carId: string): Promise<void> {
    try {
      const car = await carRepository.getCarById(carId);
      if (!car) {
        wrapErrorResponse(res, 404, 'Car with the specified ID not found');
